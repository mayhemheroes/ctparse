#!/usr/bin/env bash
#
# mayhem/build.sh — build the ctparse Atheris fuzz harness + its standalone reproducer,
...
#      then install atheris + ctparse's runtime deps + the test deps OFFLINE from that wheelhouse
#      into a fixed site dir on PYTHONPATH. The first (CI, online) build fills the wheelhouse; the
#      air-gapped PATCH re-run resolves entirely from it (pip --no-index --find-links). ctparse
#      itself stays the editable source tree (/mayhem/ctparse, exposed via PYTHONPATH, incl. its
#      bundled models/model.pbz) so a PATCH agent's edits take effect with no reinstall.
#   2. Compile launcher.c -> the ELF Mayhem target `ct_fuzzer` (Atheris is a Python
#      script; Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `ct_fuzzer-standalone`.
...
#    manylinux wheel for this CPython; pytest is the suite runner; python-dateutil/regex/tqdm are
#    ctparse's install_requires (setup.py).
PKGS=(atheris pytest "python-dateutil>=2.7.3,<3.0.0" "regex>=2018.6.6" "tqdm>=4.23.4,<5.0.0")
need_download=0
...
#    site dir holds atheris+pytest+deps we SKIP the reinstall.
...
# ctparse is a package at the repo root ($SRC/ctparse, with its bundled model); expose it (and the
# site dir) on PYTHONPATH so a PATCH agent's edits under ctparse/ take effect immediately.
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, pytest; import ctparse; print("imports OK")'
...
#    run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + ctparse.
HARNESS="$SRC/mayhem/fuzz_ct.py"
echo ">> compiling ct_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/ct_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/ct_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/ctparse_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/ct_fuzzer" "$SRC/ct_fuzzer-standalone" "$SRC/ctparse_run_tests"
