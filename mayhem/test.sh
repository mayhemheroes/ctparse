#!/usr/bin/env bash
#
# mayhem/test.sh — RUN ctparse's own pytest suite (deps already installed by mayhem/build.sh) and
...
#
# It does NOT compile — build.sh installed pytest + atheris + ctparse's deps into the in-image site
# dir and compiled the ctparse_run_tests ELF wrapper. We only RUN the suite. The suite is routed
...
# Put the in-image site dir (atheris + pytest + deps) and the ctparse source on PYTHONPATH.
...
# Run the ENTIRE upstream suite (tests/ — setup.py's test_suite). -p no:cacheprovider keeps the
# read-only image happy; write scratch (tmp) under /tmp, never the image dir.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -o cache_dir=/tmp/pytest_cache -q tests/ 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}

# Parse pytest's summary line, e.g. "89 passed in 40s" / "1 failed, 88 passed in ...".
